...
void frequente(struct Node* head) {
    ...
    while (atual != NULL) {
        if (atual->data == prev) {
            ...
            if (count > maxcount) {
                maxcount = count;
                freq = prev;
                countfreq = 1;  // Reset para 1, porque encontramos um novo número com maior frequência
            } else if (count == maxcount) {
                countfreq++;  // Encontramos outro número com a mesma frequência máxima
            }
            prev = atual->data;
            count = 1;
        }
    ...
    // Última verificação para o último elemento
    if (count > maxcount) {
        freq = prev;
        countfreq = 1;
    } else if (count == maxcount) {
